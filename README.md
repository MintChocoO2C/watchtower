...
- **Playback failure / stall**: when the player fails, or sits in its loading state (black screen with only the spinning logo) for more than 30 seconds without the video advancing, the room covers it and reloads automatically (up to 3 times in 10 minutes), then leaves a `Retry` button. Ad breaks and a manual pause are not treated as stalls. If the player just sits on its play button at `00:00`: when it has no media source (or its loading indicator is stuck) the room reloads that stream after 30 seconds; when the source is fine but playback did not start, it mutes the player and presses play for you. The reload button in the tile's hover bar reloads just that stream at any time.
