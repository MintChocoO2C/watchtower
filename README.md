...
- **Focus view**: double-click a tile (or press the expand button in its hover bar) to make that stream fill the room with sound on that channel only. Double-click again, press Esc, or hit `← Grid` at the top to return to the previous grid and sound state. While focused, `←`/`→` move to the neighbouring stream (wrapping at either end). Each press immediately flashes an arrow on that edge, shows `n / total · channel` in the toast at the top, and previews the target stream in a small corner window. Repeated presses only move the preview; once you pause, it switches to that stream in one go (Esc cancels). The pause length (0.5 s by default) is adjustable via “Arrow-key switch delay” in the settings drawer. If only one stream is live, it tells you there is nothing to switch to. In the grid, number keys 1–9 focus the n-th stream.
- **Focus view is the low-latency mode**: with several streams loading at once, every brief stall makes Safari drop low-latency playback, so latency piles up past 20 s and never recovers. Entering focus view pauses the other streams (“Pause other streams in focus view” in the settings drawer) and shows a badge in the bottom-right corner with how many seconds behind live you are (3–4 s is normal). When latency stays above the threshold (8 s by default, “Catch-up latency threshold”) for a few seconds, the stream is reloaded back to 3–4 s (a 1–2 s glitch; turn off with “Auto catch-up”). Click the badge to catch up right away. Nothing happens while you have paused the player yourself or during ads. Returning to the grid reloads the paused streams.
...
- **Loading screen**: until a stream actually starts playing, the tile shows the avatar and channel name, a progress bar, and a checklist of what is happening right now (opening player → connecting to stream → receiving video). It disappears once video is flowing.
...
- **Playback failure / stall**: when the player fails, or sits in its loading state (black screen with only the spinning logo) for more than 20 seconds without the video advancing, the room covers it and reloads automatically (up to 3 times in 10 minutes), then leaves a `Retry` button. Ad breaks and a manual pause are not treated as stalls. If the player just sits on its play button at `00:00`: when it has not received media (or its loading indicator is stuck) the room reloads that stream after 20 seconds; when media arrived but playback did not start, it mutes the player and presses play for you. The reload button in the tile's hover bar reloads just that stream at any time.
