...
- **집중 보기는 저지연 모드입니다**: 격자에서 여러 방송을 동시에 받으면 한 방송이 잠깐 멈출 때마다 Safari 가 저지연 재생을 포기해 지연이 20초 넘게 쌓이고 되돌아오지 않습니다. 집중 보기에 들어가면 나머지 방송의 스트림을 멈추고(설정 서랍 ‘집중 모드에서 나머지 방송 정지’), 화면 오른쪽 아래 배지에 지금 방송보다 몇 초 늦게 보고 있는지 보여 줍니다(정상은 3~4초). 지연이 기준(기본 8초, 설정 서랍 ‘따라잡기 기준 지연’)을 몇 초 넘게 넘으면 스트림을 다시 불러 3~4초로 되돌립니다(1~2초 끊김, ‘자동 따라잡기’ 로 끌 수 있음). 배지를 누르면 바로 따라잡습니다. 직접 일시정지했거나 광고 중일 때는 건드리지 않습니다. 격자로 나가면 멈췄던 방송을 다시 불러옵니다.
...
- **재생 실패·멈춤**: 플레이어가 재생에 실패하거나, 로딩 상태(가운데 로고만 도는 검은 화면)에서 20초 넘게 영상이 앞으로 가지 않으면 안내를 덮고 잠시 후 자동으로 다시 불러옵니다(10분에 3회까지). 그 뒤로는 `다시 시도` 버튼으로 직접 재시도합니다. 광고 중이거나 직접 일시정지한 경우는 멈춤으로 보지 않습니다. 플레이어가 재생 버튼과 `00:00`만 보인 채 시작하지 않을 때, 영상을 받지 못한 상태(또는 로딩 표시가 멈춘 상태)면 20초 뒤 그 방송만 다시 불러오고, 영상은 받았는데 재생만 안 된 상태면 음소거를 걸고 재생 버튼을 대신 눌러 줍니다. 타일 상단 바(마우스를 올리면 나옴)의 새로고침 버튼으로 언제든 그 방송만 다시 불러올 수 있습니다.
