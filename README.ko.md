...
- **마우스 제스처** — 휠(가운데) 버튼을 누른 채 드래그해서 뒤로/앞으로, 탭 조작 등을 실행. 궤적과 화살표 HUD로 실시간 피드백. (물리 마우스 전용)
...
팝업에서 **마우스 제스처** 토글 ON 후 사용. **휠(가운데) 버튼을 누른 채 드래그**하면 방향에 따라 동작이 실행됩니다. 드래그하는 동안 화면에 궤적이 그려지고, 하단 HUD에 인식된 방향(화살표)과 동작명이 실시간으로 표시됩니다.

> 트리거로 **휠 버튼**을 쓰는 이유: macOS Safari는 우클릭 메뉴를 버튼을 *누르는* 순간 띄우고, 한 번 막으면 다시 띄울 수 없어 우클릭 제스처가 불가능합니다. 휠 버튼은 이 충돌이 없어 **우클릭 메뉴와 프레임 캡처가 그대로 유지**됩니다. (물리 마우스 전용 — 트랙패드에는 휠 버튼이 없습니다.)
...
그냥 휠클릭(드래그 없이)은 평소대로 동작합니다(링크는 새 탭으로 열림).

## 기술 스택

- **Swift** — 호스트 앱 + 확장 엔트리 (`SafariWebExtensionHandler`)
- **JavaScript** — `manifest_version: 3` 웹 확장 (background, wt-core, content, vimium, gestures, page-script, popup)
- **Xcode** — 빌드 및 패키징

확장은 3개 실행 컨텍스트 사용:
- 서비스 워커 (`background.js`) — 컨텍스트 메뉴, 메시지 라우팅, 스토리지 relay 담당
- isolated content scripts (`wt-core.js` → `content.js` → `vimium.js` → `gestures.js`) — DOM 접근 가능; 페이지 전역 변수 접근 불가. `wt-core.js`가 가장 먼저 로드되어 공용 서비스(`window.WT`: 로깅·설정 구독·탭 조작)를 노출하고 나머지 기능이 이를 공유
