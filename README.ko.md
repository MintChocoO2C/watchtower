...
- **재생 실패·멈춤**: 플레이어가 재생에 실패하거나, 로딩 상태(가운데 로고만 도는 검은 화면)에서 30초 넘게 영상이 앞으로 가지 않으면 안내를 덮고 잠시 후 자동으로 다시 불러옵니다(10분에 3회까지). 그 뒤로는 `다시 시도` 버튼으로 직접 재시도합니다. 광고 중이거나 직접 일시정지한 경우는 멈춤으로 보지 않습니다. 플레이어가 재생 버튼과 `00:00`만 보인 채 시작하지 않을 때, 영상 소스를 얻지 못한 상태(또는 로딩 표시가 멈춘 상태)면 30초 뒤 그 방송만 다시 불러오고, 소스는 있는데 재생만 안 된 상태면 음소거를 걸고 재생 버튼을 대신 눌러 줍니다. 타일 상단 바(마우스를 올리면 나옴)의 새로고침 버튼으로 언제든 그 방송만 다시 불러올 수 있습니다.
