...
- **JavaScript** — `manifest_version: 3` 웹 확장 (background, wt-core, content, page-script, popup)
- **Xcode** — 빌드 및 패키징

확장은 3개 실행 컨텍스트 사용:
- 서비스 워커 (`background.js`) — 컨텍스트 메뉴, 메시지 라우팅, 스토리지 relay 담당
- isolated content scripts (`wt-core.js` → `content.js`) — DOM 접근 가능; 페이지 전역 변수 접근 불가. `wt-core.js`가 가장 먼저 로드되어 공용 서비스(`window.WT`: 로깅·설정 구독)를 노출하고 나머지 기능이 이를 공유
